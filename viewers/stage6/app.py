...
# Обновили путь по умолчанию на stage6
DEFAULT_INPUT_FILE = PROJECT_ROOT / "parsed_danluu" / "danluu_postmortems_stage6.jsonl"

INPUT_FILE = Path(os.getenv("STAGE6_JSONL_PATH", str(DEFAULT_INPUT_FILE)))
...
def build_search_text(row: dict) -> str:
    ...
    stage6 = get_stage6(row) or {}
    
    extraction = stage6.get("extraction") or {}
    ...
    parts = [
        ...
        # Добавляем Stage 6 данные в полнотекстовый поиск вьюера
        safe_str(extraction.get("company")),
        safe_str(extraction.get("short_description")),
    ]
    
    # Добавим теги стека для удобного поиска
    meta_filters = extraction.get("metadata_filters", {})
    if isinstance(meta_filters.get("tech_stack"), list):
        parts.extend(meta_filters["tech_stack"])
...
def normalize_record(row: dict, row_index: int) -> dict:
    ...
    stage6_status = classify_stage_status(stage6)
    
    extraction = stage6.get("extraction") if stage6 else None
...
def compute_meta(records: list[dict]) -> dict:
    ...
    stage6_counter = Counter(record["stage6_status"] for record in records)
    ...
    return {
        ...
        "stage6_status_counts": dict(stage6_counter),
...
def record_matches_filters(
    ...
    *,
    ...
    if stage6_status != "all" and record["stage6_status"] != stage6_status:
        return False
...
def serialize_sidebar_item(record: dict, filtered_index: int) -> dict:
    return {
        ...
        "stage6_status": record["stage6_status"],
...
def serialize_current_record(record: dict, filtered_index: int) -> dict:
    ...
    return {
        ...
        "stage6_extraction_json": json.dumps(record["stage6_extraction"], ensure_ascii=False, indent=2) if record["stage6_extraction"] else None,
...
def api_query():
    ...
    stage6_status = safe_str(payload.get("stage6_status", "all")) or "all"
