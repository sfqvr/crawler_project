...
const dom = {
  ...
  statHtmlLength: document.getElementById("statHtmlLength"),
...
function renderSidebarItems(items, selectedIndex) {
  ...
  items.forEach((item) => {
    ...
    const markdownTag = item.has_markdown ? "md✓" : "md✗";
    ...
    button.innerHTML = `
      ...
      <div class="document-item-meta">
        <span>${escapeHtml(kind)}</span>
        <span>·</span>
        <span>${escapeHtml(markdownTag)}</span>
        <span>·</span>
        <span>stage4=${escapeHtml(item.stage4_status)}</span>
        <span>·</span>
        <span>stage5=${escapeHtml(item.stage5_status)}</span>
      </div>
    `;
...
function renderBadges(current) {
  ...
  if (current.stage4_relevant === true) {
    badges.push(`<span class="badge badge-green">stage4 relevant</span>`);
  } else if (current.stage4_relevant === false) {
    badges.push(`<span class="badge badge-red">stage4 not relevant</span>`);
  }

  if (current.stage4_can_extract_markdown === true) {
    badges.push(`<span class="badge badge-green">can extract markdown</span>`);
  } else if (current.stage4_can_extract_markdown === false) {
    badges.push(`<span class="badge badge-yellow">cannot extract markdown</span>`);
  }

  if (current.has_markdown) {
    badges.push(`<span class="badge badge-green">markdown present</span>`);
  } else {
    badges.push(`<span class="badge badge-yellow">no markdown</span>`);
  }
...
function renderCurrent(current) {
  ...
  dom.statHtmlLength.textContent = String(current.cleaned_html_length ?? 0);
